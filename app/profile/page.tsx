          ...
          <div className="text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono">{`${userData.walletAddress.slice(
                  0,
                  6
                )}...${userData.walletAddress.slice(-4)}`}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {userData.registeredIps && userData.registeredIps.length > 0 ? (
                ...
                <Card className="neo-card overflow-hidden">
                  ...
                  <CardContent className="p-6 space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <Button
                        variant="secondary"
                        className="w-full bg-green-500/10 hover:bg-green-500/20 text-green-500 border border-green-500/20"
                        onClick={async () => {
                          try {
                            toast.loading("Claiming revenue...", {
                              id: "claim-" + ip.ipId,
                            });
                            const res = await fetch("/api/claim-revenue", {
                              method: "POST",
                            ...
                            if (data.success) {
                              let claimedAmounts = "";
                              // Handle both array (Event objects) and object ({token: amount}) formats
                              if (Array.isArray(data.claimedTokens)) {
                                claimedAmounts = data.claimedTokens
                                  .map((t: any) => `${t.amount} WIP`)
                                  .join(", ");
                              } else {
                                claimedAmounts = Object.entries(
                                  data.claimedTokens
                                )
                                  .map(([token, amount]) => `${amount} WIP`)
                                  .join(", ");
                              }
                              toast.success(
                      ...
                      >
                        <Wallet className="mr-2 h-4 w-4" /> Claim Revenue
                      </Button>
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="text-center py-12 border-2 border-dashed border-muted rounded-xl">
            ...
            <p className="text-muted-foreground mt-2">
              Start creating and register your first IP!
            </p>
            <Link href="/generate">
